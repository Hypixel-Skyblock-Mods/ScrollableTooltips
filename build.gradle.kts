...
val targets = targetNames.associateWith { name ->
    Target(minecraft = targetProperties.required("$name.minecraft"))
}
val modVersion = providers.gradleProperty("mod_version").get()

allprojects {
    group = "org.hypixelskyblockmods.scrollabletooltips"
    version = modVersion
...
subprojects {
    ...
    extensions.configure<ModrinthExtension> {
        ...
        versionName.set("Scrollable Tooltips $artifactVersion")
...
val releaseTargets = targets.map { (name, target) ->
    ...
    linkedMapOf(
        ...
        "jar" to "versions/$name/build/libs/ScrollableTooltips-$artifactVersion.jar",
    )
}
...
data class Target(val minecraft: String)
